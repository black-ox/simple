...
import kafka.common.{NoEpochForPartitionException, TopicAndPartition}
...
object ZkUtils extends LogSupport {
  ...
  def getSortedBrokerList(zkClient: ZkClient): Seq[Int] =
    getChildren(zkClient, BrokerIdsPath).map(_.toInt).sorted
  ...
  def conditionalUpdatePersistentPath(client: ZkClient, path: String, data: String, expectVersion: Int,
    ...
    try {
      ...
      LOG.debug("Conditional update of path %s with value %s and expected version %d succeeded, returning the new version: %d"
      ...
      case e1: ZkBadVersionException =>
        optionalChecker match {
          ...
          case _ => LOG.debug("Checker method is not passed skipping zkData match")
        }
        LOG.warn("Conditional update of path %s with data %s and expected version %d failed due to %s".format(path, data,
  ...
  def conditionalUpdatePersistentPathIfExists(client: ZkClient, path: String, data: String, expectVersion: Int): (Boolean, Int) = {
    try {
      ...
      case e: Exception =>
        LOG.error("Conditional update of path %s with data %s and expected version %d failed due to %s".format(path, data,
            ...
            case Some(m) => m.asInstanceOf[Map[String, Any]].get("partitions") match {
              case Some(repl) =>
                ...
                for ((partition, replicas) <- replicaMap) {
                  ...
                  LOG.debug("Replicas assigned to topic [%s], partition [%s] are [%s]".format(topic, partition, replicas))
                }
              case None =>
            }
            case None =>
  ...
  def updatePartitionReassignmentData(zkClient: ZkClient, partitionsToBeReassigned: Map[TopicAndPartition, Seq[Int]]) {
    ...
    partitionsToBeReassigned.size match {
      case 0 => // need to delete the /admin/reassign_partitions path
        ...
        LOG.info("No more partitions need to be reassigned. Deleting zk path %s".format(zkPath))
      case _ =>
        val jsonData = getPartitionReassignmentZkData(partitionsToBeReassigned)
        try {
          updatePersistentPath(zkClient, zkPath, jsonData)
          LOG.info("Updated partition reassignment path with %s".format(jsonData))
        } catch {
          case nne: ZkNoNodeException =>
            ZkUtils.createPersistentPath(zkClient, zkPath, jsonData)
            LOG.debug("Created path %s with %s for partition reassignment".format(zkPath, jsonData))
